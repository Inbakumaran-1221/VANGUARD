...
function computeGapScore(stop) {
  ...
  const evidenceSeverity = (stop.evidence ?? []).reduce((sum, item) => {
  ...
  // Improved formula: Gap Score = Missing Features + Severity Weight.
  const severityWeight = Math.min(35, Math.round(stop.grievanceCount * 0.45 + evidenceSeverity));
...
function clusterGrievances(rawText) {
  ...
  lines.forEach((line) => {
    ...
    if (!matched) themes.signage.count += 1;
  });
...
export function getAnalytics() {
  ...
  const scoredStops = state.stops
    .map((stop) => {
      const { gapScore, missingFeatures, severityWeight } = computeGapScore(stop);
