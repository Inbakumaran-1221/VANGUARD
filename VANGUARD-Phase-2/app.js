...
function clusterGrievances(rawText) {
  ...
  const themes = {
    ramp: { label: "Ramp / Wheelchair Access", keywords: ["ramp", "wheelchair", "elevator", "boarding"], count: 0 },
    audio: { label: "Audio Signals", keywords: ["audio", "announcement", "voice", "audible"], count: 0 },
    tactile: { label: "Tactile Path", keywords: ["tactile", "paving", "surface"], count: 0 },
    signage: { label: "Signage & Wayfinding", keywords: ["signage", "visual", "display", "confusing"], count: 0 }
  };
...
function renderCategoryBreakdown(stops) {
  const totals = {
    ramp: stops.filter((stop) => !stop.hasRamp).length,
    tactile: stops.filter((stop) => !stop.hasTactile).length,
    audio: stops.filter((stop) => !stop.hasAudio).length,
    lowFloor: stops.filter((stop) => !stop.hasLowFloorAccess).length
  };

  const items = [
    { label: "Ramp gaps", value: totals.ramp },
    { label: "Tactile path gaps", value: totals.tactile },
    { label: "Audio signal gaps", value: totals.audio },
    { label: "Low-floor boarding gaps", value: totals.lowFloor }
  ];
...
function refreshDashboard() {
  ...
  const { themeStats } = clusterGrievances(grievanceInput.value);
  const coherence = renderClusters(themeStats, grievanceLines.length);
  const scored = enrichStops(currentStops);

  renderScoreTable(scored);
  renderMap(scored);
  renderDistribution(scored);
  renderCategoryBreakdown(scored);
