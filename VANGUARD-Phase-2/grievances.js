...
function renderCategoryBreakdown(scoredStops) {
  const totals = {
    ramp: scoredStops.filter((stop) => !stop.hasRamp).length,
    tactile: scoredStops.filter((stop) => !stop.hasTactile).length,
    audio: scoredStops.filter((stop) => !stop.hasAudio).length,
    lowFloor: scoredStops.filter((stop) => !stop.hasLowFloorAccess).length
  };
...
function refresh() {
  ...
  renderCategoryBreakdown(data.scoredStops);
