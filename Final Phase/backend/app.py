...
def _derive_gap_messages(features: dict[str, Any], has_accessibility_match: bool) -> tuple[list[str], list[str]]:
    ...
    if not has_accessibility_match:
        ...
        return gap_messages, suggestions

    if features.get("ramp") is False:
        gap_messages.append("Ramp missing")
        suggestions.append("Install accessible wheelchair boarding ramp")
    if features.get("stairs") is True and features.get("ramp") is False:
        gap_messages.append("Stairs detected without accessible ramp")
        suggestions.append("Retrofit stairs with an adjacent ADA-compliant ramp")
    if features.get("tactile") is False:
        gap_messages.append("Tactile path not detected")
        suggestions.append("Lay tactile paving leading to platform edge")
    if features.get("braille") is False:
        gap_messages.append("Braille signage not detected")
        suggestions.append("Add verified braille information boards")
...
def analyze() -> tuple[dict[str, Any], int]:
    ...
    gap_messages, suggestion_messages = _derive_gap_messages(features, has_accessibility_match)
...
@app.post("/ml-hook")
def ml_hook() -> tuple[dict[str, Any], int]:
    import time
    import random
    ...
    # Simulate ML processing latency
    time.sleep(0.5)

    barriers = ["Ramp Missing", "No Audio Signals", "Blocked Pathway", "No Tactile Paving"]
    themes = ["Mobility Accessibility", "Audio Accessibility", "Path Clearance", "Visual Accessibility"]
    
    pick = random.randint(0, 3)

    return jsonify({
        "detected_barrier": barriers[pick],
        "grievance_theme": themes[pick],
        "severity_score": random.randint(60, 95),
        "text_received": text_payload
    }), 200
